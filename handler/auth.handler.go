...
func (ah *AuthHandler) homeHandler(c echo.Context) error {
	homeView := auth_view.Home(fromProtected)
	isError = false
...
func (ah *AuthHandler) registerHandler(c echo.Context) error {
	registerView := auth_view.Register(fromProtected)
	isError = false
	...
	return renderView(c, auth_view.RegisterIndex(
		"| Register",
		"",
		fromProtected,
		isError,
...
func (ah *AuthHandler) loginHandler(c echo.Context) error {
	loginView := auth_view.Login(fromProtected)
	isError = false
	...
	if c.Request().Method == "POST" {
		...
		return c.Redirect(http.StatusSeeOther, "/notready/comebacklater")
	}

	return renderView(c, auth_view.LoginIndex(
		"| Login",
		"",
		fromProtected,
		isError,
